...
export class LocalStorageService extends AbstractStorageService {
  ...
  async exists(key: string): Promise<StorageExistsResult> {
    try {
      ...
      this.logger.error(
        `Failed to check existence of file ${key}: ${error.message}`,
        error.stack,
      );
      return {
        exists: false,
  ...
  async getUrl(
    ...
    try {
      ...
      this.logger.error(
        `Failed to generate URL for file ${key}: ${error.message}`,
        error.stack,
      );
      return {
        url: '',
  ...
  async copy(
    ...
    try {
      ...
      this.logger.error(
        `Failed to copy file from ${fromKey} to ${toKey}: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async getMetadata(key: string): Promise<{
    ...
    try {
      ...
      this.logger.error(
        `Failed to get metadata for file ${key}: ${error.message}`,
        error.stack,
      );
      return null;
    }
  ...
  private generateLocalUrl(key: string): string {
    ...
    if (!baseUrl) {
      ...
      if (configuredUrl) {
        ...
        // Use configured backend URL or construct from environment
